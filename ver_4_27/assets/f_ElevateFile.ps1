...
for /f usebackq^ tokens^=*^ delims^=^ eol^= %%x in (`powerShell -ExecutionPolicy RemoteSigned -Command ""%~dp0\assets\f_ElevateFile.ps1" 0 %*"`) do (
	if "%%x" equ "True" (
		echo(Was Elevated
	) else (
...
```
rem //# Elevate if wasn't
powerShell -ExecutionPolicy RemoteSigned -Command ""%~dp0\assets\f_ElevateFile.ps1" "%~f0" %*" >nul
```
#>
...
if ($cont_1 -eq 0) {
	...
	if ( ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] 'Administrator') ) {
		"0 f_ElevateFile: Didn't Elevate"		# out
	} else {
		# [dev] [1]
		if ([int](Get-CimInstance -Class Win32_OperatingSystem | Select-Object -ExpandProperty BuildNumber) -ge 6000) {
			...
			'1 f_ElevateFile: Did Elevate'		# out
		} else {
			# [dev] [=1]
			"2 f_ElevateFile: Can't elevate"		# out
		}
	}
}
